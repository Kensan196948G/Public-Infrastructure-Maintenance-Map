---
name: "source-command-phase-loop"
description: "Goal Rotation の現在フェーズ (Monitor/Development/Verify/Improvement) を1つ実行し、充足時にポインタを前進する"
---

# source-command-phase-loop

Use this skill when the user asks to run the migrated source command `phase-loop`.
...
1. 📊 **現在フェーズの確認** — `state.json` の `goal_rotation` を Read する。
   ...
   - `blocked=true` の場合は実行せず、ブロック理由 (`state.warnings` の `goal_rotation_blocked`) を
     ユーザーに提示して停止する。
2. 📄 **フェーズ定義の読込** — `.Codex/goal/` から `current` に対応するファイルを Read する。
   | current | ファイル |
   |---|---|
   | monitor | `.Codex/goal/10-monitor.md` |
   | development | `.Codex/goal/20-development.md` |
   | verify | `.Codex/goal/30-verify.md` |
   | improvement | `.Codex/goal/40-improvement.md` |
...
5. ⏭️ **ポインタ前進** — 充足時のみ以下を実行する (手動セッションでは launcher finalize が
   ...
   ```bash
   node .Codex/claudeos/scripts/hooks/goal-rotation.js advance --manual
   ```
