...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--dry-run') args.dryRun = true;
      ...
      console.log(
        'Usage: node scripts/tools/run-ultrareview.js [--target <pr#|branch>] [--dry-run] [--timeout <min>]',
      );
      process.exit(0);
    }
  }
...
function main() {
  ...
  if (!cap.allowed) {
    console.log(
      `[ultrareview] SKIP: monthly cap reached (${cap.used}/${cap.cap}) for ${cap.ur.current_month}`,
    );
    process.exit(0);
  }
  ...
  if (args.dryRun) {
    ...
    console.log(
      `[ultrareview][DRY-RUN] would save to: ${path.join(REPORT_DIR, `${todayStamp()}.json`)}`,
    );
  ...
  if (severeFindings.length > 0) {
    ...
    console.log(
      `[ultrareview][WARN] ${severeFindings.length} critical/high finding(s); recorded to state.warnings`,
    );
  } else {
