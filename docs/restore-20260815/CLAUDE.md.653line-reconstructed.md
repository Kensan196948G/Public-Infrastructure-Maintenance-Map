# 再構成ドラフト: CLAUDE.md（653行版）— 復元版

> **ステータス: ドラフト（ユーザー確認待ち）**
>
> - 1〜200行: `CLAUDE.md.653line-read-notes.md` から**原文そのまま**復元（セクション1〜9）
> - 201行以降: 破棄された原文は復元不能のため、プロジェクト実態（本リポジトリの運用・CI・デプロイ・監査・過去の記録）に基づき**再構成したもの**。ユーザー確認の上で修正・確定する。
> - 確認後、ルート `CLAUDE.md` へ適用するか、内容を差し替えて再提供いただく。
